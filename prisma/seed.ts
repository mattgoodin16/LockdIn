...
import { GoalType, PrismaClient, TrainingLevel } from "@prisma/client";
import { Pool } from "pg";
...
async function seedUser(email: string, name: string, goalType: GoalType) {
  ...
  await prisma.userProfile.upsert({
    ...
    create: {
      ...
      dietaryPreference: "OMNIVORE", // string literal
  ...
  const goal = await prisma.goalProfile.upsert({
    ...
    create: {
      ...
      equipmentType: "DUMBBELLS_ONLY", // string literal
      workoutsPerWeek: 4,
  ...
  await prisma.bodyMetricLog.createMany({
    data: [
      { userId: user.id, loggedAt: new Date(Date.now() - 35 * 24 * 3600 * 1000), weightKg: 89.8, waistCm: 100, energyLevel: 6, sleepQuality: 6 },
      { userId: user.id, loggedAt: new Date(Date.now() - 7 * 24 * 3600 * 1000), weightKg: 87.7, waistCm: 96, energyLevel: 7, sleepQuality: 7 },
    ],
  });

  await prisma.nutritionLog.create({
    data: { userId: user.id, loggedAt: new Date(), macroAdherencePercent: 84, calorieAdherencePercent: 82, waterLiters: 3.1, notes: "Solid adherence with one social meal." },
  });

  await prisma.workoutLog.create({
    data: { userId: user.id, workoutPlanId: plan.id, completed: true, completionPercent: 100, difficultyRating: 7, notes: "Progressed top sets with clean form.", performedAt: new Date() },
  });

  await prisma.progressPhoto.createMany({
    data: [
      { userId: user.id, label: "front", imageUrl: "/placeholder/front-week1.jpg", takenAt: new Date(Date.now() - 21 * 24 * 3600 * 1000) },
      { userId: user.id, label: "front", imageUrl: "/placeholder/front-week4.jpg", takenAt: new Date(Date.now() - 1 * 24 * 3600 * 1000) },
    ],
  });
  ...
  await prisma.weeklyRecap.create({
    data: {
      ...
      shareableCardPayload: { style: "neon-premium", cards: ["workouts", "adherence", "streak"] },
    },
  });
...
main()
  ...
  .catch(async (e) => { console.error(e); await prisma.$disconnect(); process.exit(1); });    description: "Joint-friendly horizontal pressing variation.",
    executionNotes: ["Neutral wrist", "Slight arch", "Lockout softly"],
    muscleGroups: ["chest", "triceps", "front-delts"],
    movementPattern: "horizontal_push",
    equipment: ["FULL_GYM", "HOME_GYM", "DUMBBELLS_ONLY"],
